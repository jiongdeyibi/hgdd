...
public final class Request2ModelUtil {
    ...
    public static final <K> K covert(Class<K> T, HttpServletRequest request) {
        try {
            ...
            while (methodIterator.hasNext()) {
                ...
                Type[] typeGen = method.getGenericParameterTypes();
                if (typeGen != null && typeGen[0] instanceof ParameterizedType) {
                    if (((ParameterizedType) typeGen[0]).getActualTypeArguments() != null) {
                        String actualTypeArgument = (((ParameterizedType) typeGen[0]).getActualTypeArguments())[0].toString();
                        String className = actualTypeArgument.substring(TYPE_NAME_PREFIX.length());
                        if (StringUtil.isNotEmpty(className)) {
                            model = Class.forName(className);
                        }
                    }
                }
                Object[] param_value = new Object[]{TypeParseUtil.convert(value, type[0], null, model)};
    ...
    public static final <K> void getMethodParas(Class<K> T, HttpServletRequest request, Set<String> set) {
    ...
    public static final <K> void getOtherFormMap(Class<K> T, HttpServletRequest request, Map<String, String> formMap) {

        try {
            Set<String> methodParasSet = new HashSet<String>();
            Request2ModelUtil.getMethodParas(T, request, methodParasSet);
            ...
            for (Enumeration<String> e = paraNames; e.hasMoreElements(); ) {
                ...
                String thisValue = "";
                String[] thisValueArray = request.getParameterValues(thisName);
                if (thisValueArray != null || thisValueArray.length > 0) {
                    thisValue = StringUtils.join(thisValueArray, ',');
                } else {
                ...
                if (!methodParasSet.contains(thisName)) {
                    formMap.put(thisName, thisValue);
                }
            }
        } catch (Exception ex) {
        }
        ...
        while (iterator.hasNext()) {
            ...
            try {
                ...
                if (method != null) {
                    ...
                    if (method != null) {
                        Object[] param_value = new Object[]{TypeParseUtil.convert(value, parameterTypes[0], null, null)};
